...
### Changed
- **`src/core/types.ts`**：扩展为 shared/Types.ts 的完整版本（补 skills/frozen/killStreak）
- **`src/core/utils.ts`**：新增 resetIdCounter() 函数
- **计分系统**：吃小兵+1分，吃马+3分，吃车+5分

### Removed
- **金币系统**：移除棋盘上的金币生成、拾取和渲染逻辑，得分仅通过击杀敌人获得
